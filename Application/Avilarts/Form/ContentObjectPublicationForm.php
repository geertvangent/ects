...
class ContentObjectPublicationForm extends FormValidator
{
    ...
    public function build_create_form()
    {
        ...
        if (count($this->publications) == 1)
        {
            ...
            if ($first_publication && $first_publication->get_content_object()->is_complex_content_object() && ($first_publication->get_content_object()->get_owner_id() ==
                 Session :: get_user_id() || RepositoryRights :: get_instance()->is_allowed_in_user_subtree(
                    RepositoryRights :: COLLABORATE_RIGHT,
                    $first_publication->get_content_object()->get_id(),
                    RepositoryRights :: TYPE_USER_CONTENT_OBJECT,
                    $first_publication->get_content_object()->get_owner_id())))
            {

                /*
                 * only display publish and build if the complex content object has no content otherwise only display
                 * publish button exceptions are added to 'only display publish and build if the complex content object
                 * has no content'
                 */
                ...
                $condition = new EqualityCondition(
                    ...
                    new StaticConditionVariable($first_publication->get_content_object_id()));

                $has_content = \Chamilo\Core\Repository\Storage\DataManager :: count_complex_content_object_items(
                    \Chamilo\Core\Repository\Storage\DataClass\ComplexContentObjectItem :: class_name(),
                    $condition);
                ...
                $has_display = class_exists($class);

                if (! $has_content || ! $has_display)
                {
                    $buttons[] = $this->createElement(
                        'style_submit_button',
    ...
    public function collaborate_content_object_with_course_admins(ContentObject $content_object)
    {
        ...
        if ($content_object && ($content_object->get_owner_id() == Session :: get_user_id()))
        {
            ...
            foreach ($admin_users as $admin_user)
            {
                ...
                {
                    $succes &= $content_object->share_with_user(
                        $admin_user[User :: PROPERTY_ID],
                        RepositoryRights :: COLLABORATE_RIGHT);
                }
            }
            ...
            {
                $succes &= $content_object->share_with_group(
                    $admin_group[Group :: PROPERTY_ID],
                    RepositoryRights :: COLLABORATE_RIGHT);
            }
        }
