...
class ManageForm extends FormValidator
{
    ...
    private function build_right_form()
    {
        ...
        $this->addElement(
            ...
            '', 
            array(
                self :: ACTION_GRANT => Translation :: get('Grant'), 
        ...
        $this->addElement(
            ...
            ResourceManager :: get_instance()->get_resource_html(
                Path :: get_common_extensions_path(true) .
                     'new_rights_editor_manager/resources/javascript/rights_form.js'));
    }
            ...
            foreach ($values[self :: PROPERTY_RIGHT] as $right_id)
            {
                switch ($option)
                {
                    case self :: RIGHT_OPTION_ALL :
                        ...
                        {
                            $succes &= $rights->unset_location_entity_right(
                                $this->context, 
                                $right_id, 
                                0, 
                                0, 
                    ...
                    case self :: RIGHT_OPTION_ME :
                        if ($action == self :: ACTION_GRANT)
                        {
                            $succes &= $rights->set_location_entity_right(
                                $this->context, 
                                $right_id, 
                                Session :: get_user_id(), 
                                1, 
