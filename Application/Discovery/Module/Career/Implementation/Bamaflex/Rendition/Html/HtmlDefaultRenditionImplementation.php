    ...
    public function get_enrollment_courses($selected_contract)
    {
        ...
        foreach ($years as $year => $types)
        {
            foreach ($types as $type => $credits)
            {
                $row = array();
                $row[] = $year;
                $row[] = '<img src="' . Theme :: getInstance()->getImagePath() . 'course_type/' . $type . '.png" alt="' .
                ...
                if (in_array($type, Course :: get_types_for_total_credits()))
                {
                    ...
                    {
                        $row[] = '<img src="' . Theme :: getInstance()->getImagePath() . 'total_type/3.png" alt="' .
                             Translation :: get('CreditPending') . '" title="' . Translation :: get('CreditPending') .
                             '" />';
                    }
                    else
                    {
                        $row[] = '<img src="' . Theme :: getInstance()->getImagePath() . 'total_type/1.png" alt="' .
                ...
                {
                    $row[] = '<img src="' . Theme :: getInstance()->getImagePath() . 'total_type/2.png" alt="' .
    ...
    public function get_table_data($enrollment_ids, $enrollment)
    {
        ...
        foreach ($course[$enrollment->get_id()] as $course)
        {
            ...
            if ($course->is_special_type())
            {
                ...
                $course_type_image = '<img src="' . Theme :: getInstance()->getImagePath() . 'course_type/' . $course->get_type() .
            ...
            foreach ($this->get_mark_moments() as $mark_moment)
            {
                ...
                if ($mark->get_publish_status() == 1 || ! $training->is_current() || $this->result_right)
                {
                    ...
                    if ($mark->get_status())
                    {
                        ...
                        {
                            $mark_status_image = '<img src="' . Theme :: getInstance()->getImagePath() . 'status_type/' .
                ...
                else
                {
                    if ($mark->get_result())
                    {
                        $result_na_image = '<img src="' . Theme :: getInstance()->getImagePath() . 'mark_result_na.png" alt="' .
            ...
            if ($course->has_children())
            {
                foreach ($course->get_children() as $child)
                {
                    ...
                    if ($child->is_special_type())
                    {
                        ...
                        $child_type_image = '<img src="' . Theme :: getInstance()->getImagePath() . 'course_type/' .
