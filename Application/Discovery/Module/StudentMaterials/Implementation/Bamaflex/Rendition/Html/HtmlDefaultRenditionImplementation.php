...
class HtmlDefaultRenditionImplementation extends RenditionImplementation
{
    ...
    public function render()
    {
        ...
        $years = DataManager::getInstance($this->get_module_instance())->retrieve_years($this->get_module_parameters());
        if (count($years) > 0)
        {
            ...
            if ($course->has_children())
            {
                foreach ($course->get_children() as $child)
                {
                    ...
                    foreach ($materials as $material)
                    {
                        ...
                        if ($material->get_for_sale())
                        {
                            $image = '<img src="' . Theme::getInstance()->getImagePath() . 'Material/ForSale.png" alt="' .
