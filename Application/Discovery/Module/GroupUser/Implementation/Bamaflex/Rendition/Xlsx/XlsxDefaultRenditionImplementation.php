            ...
            foreach ($data as $group_user)
            {
                ...
                foreach ($group_user as $group_user_info)
                {
                    $this->php_excel->getActiveSheet()->setCellValueByColumnAndRow(
                        $column ++,
                        $row,
                        ExcelExport :: transcode_string($group_user_info));
                }

                $this->php_excel->getActiveSheet()->setCellValueByColumnAndRow(
                    $column ++,
                    $row,
                    ExcelExport :: transcode_string(Translation :: get('Enrollment')));
            ...
            if (count($course_data) > 0)
            {
                foreach ($course_data as $group_user)
                {
                    ...
                    $this->php_excel->getActiveSheet()->setCellValueByColumnAndRow(
                        $column ++,
                        $row,
                        ExcelExport :: transcode_string(Translation :: get('Course')));
