...
class DataClassGenerator
{
    ...
    public function generate_data_class($dom_xpath)
    {
        ...
        $query = "SELECT column_name, data_type FROM [INFORDATSYNC].[INFORMATION_SCHEMA].[COLUMNS] WHERE table_name LIKE '" . $content_object->getAttribute(
        ...
        if (! $results instanceof MDB2_Error)
        {
            ...
            if (! is_dir(
                    $location . Path :: namespace_to_path($content_object->getAttribute('namespace')) . '/php/lib/'))
                mkdir($location . Path :: namespace_to_path($content_object->getAttribute('namespace')) . '/php/lib/',
                        0777, true);

            $file = fopen(
                    $location . Path :: namespace_to_path($content_object->getAttribute('namespace')) . '/php/lib/' . $content_object->getAttribute(
                            'name') . '.class.php', 'w+');
            ...
            if ($file)
            {
                ...
                $this->template->assign_vars(
                        array('PACKAGE' => str_replace('\\', '.', $content_object->getAttribute('namespace')),
                                'AUTHOR' => $content_object->getAttribute('author'),
                                'OBJECT_CLASS' => Utilities :: underscores_to_camelcase(
                                        $content_object->getAttribute('name')),
                ...
                foreach ($columns as $property_name => $property_type)
                {
                    ...
                    $this->template->assign_block_vars("CONSTS",
                    ...
                    $this->template->assign_block_vars("PROPERTY",
                            array('PROPERTY_CONST' => $property_const, 'PROPERTY_NAME' => $property_name,
                                    'PROPERTY_TYPE' => $property_type));

                    $property_constants = $dom_xpath->query(
                            '/object/properties/property[@name="' . $property_name . '"]/option');
                    if ($property_constants->length > 0)
                    {
                        $this->template->assign_block_vars("PROPERTY_CONSTS",
                                array('NAME' => strtoupper($property_name)));
                        foreach ($property_constants as $property_constant)
                        {
                            $this->template->assign_block_vars("PROPERTY_CONSTS.OPTION",
                                    array('OPTION' => strtoupper($property_constant->nodeValue),
                                            'VALUE' => $property_constant->getAttribute('id')));
                        }

                        $type_name = str_replace('_id', '', $property_name);

                        $this->template->assign_block_vars("TYPES",
                                array('NAME' => strtoupper($type_name), 'FUNCTION' => $type_name));
                        foreach ($property_constants as $property_constant)
                        {
                            $this->template->assign_block_vars("TYPES.OPTIONS",
                                    array('OPTION' => strtoupper($property_constant->nodeValue),
