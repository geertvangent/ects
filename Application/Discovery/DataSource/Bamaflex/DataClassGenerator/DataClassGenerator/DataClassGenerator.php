...
class DataClassGenerator
{
    ...
    public function generate_data_class($dom_xpath)
    {
        ...
        $query = "SELECT column_name, data_type FROM [INFORDATSYNC].[INFORMATION_SCHEMA].[COLUMNS] WHERE table_name LIKE '" .
             $content_object->getAttribute('table') . "' ORDER BY ordinal_position";
            ...
            if ($file)
            {
                ...
                $this->template->assign_vars(
                    array(
                        ...
                        'OBJECT_CLASS' => StringUtilities :: getInstance()->createString(
                            $content_object->getAttribute('name'))->upperCamelize(),
                    ...
                    if ($property_constants->length > 0)
                    {
                        ...
                        foreach ($property_constants as $property_constant)
                        {
                            $this->template->assign_block_vars(
                                ...
                                array(
                                    'OPTION' => strtoupper($property_constant->nodeValue),
                                    'TRANSLATION' => StringUtilities :: getInstance()->createString($type_name)->upperCamelize() .
                                         $property_constant->nodeValue));
                        }
                    }
                }
