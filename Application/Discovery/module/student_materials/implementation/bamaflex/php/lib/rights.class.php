...
class Rights extends RightsUtil
{
    ...
    const TYPE_STUDENT_MATERIALS = 1;
    ...
    function module_is_allowed($right, $entities, $module_instance_id, $parameters)
    {
        try
        {
            ...
            {
                return parent :: is_allowed($right, 'discovery_' . $module_instance_id, null, $entities, $parameters->get_user_id(), self :: TYPE_STUDENT_MATERIALS, 0, self :: TREE_TYPE_ROOT);
            }
        }
    ...
    {
        return parent :: get_location_by_identifier('discovery_' . $module_instance_id, self :: TYPE_STUDENT_MATERIALS, $parameters->get_user_id(), 0, self :: TREE_TYPE_ROOT);
    }

    function get_module_location_id_by_identifier($module_instance_id, $parameters)
    {
        return parent :: get_location_id_by_identifier('discovery_' . $module_instance_id, self :: TYPE_STUDENT_MATERIALS, $parameters->get_user_id(), 0, self :: TREE_TYPE_ROOT);
    }

    function create_module_location($module_instance_id, $parameters, $parent)
    {
        return parent :: create_location('discovery_' . $module_instance_id, self :: TYPE_STUDENT_MATERIALS, $parameters->get_user_id(), 1, $parent, 0, 0, self :: TREE_TYPE_ROOT);
    }
    ...
    {
        return parent :: get_target_entities(self :: VIEW_RIGHT, 'discovery_' . $module_instance_id, $parameters->get_user_id(), self :: TYPE_STUDENT_MATERIALS);
    }
}
?>