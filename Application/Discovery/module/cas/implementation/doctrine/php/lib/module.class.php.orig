...
class Module extends \application\discovery\module\cas\Module
{
    ...
    function get_statistics_table($action)
    {
        ...
        if (count($action_statistics) == 1 && count($action_statistics[0]) > 0)
        {
            $path = Path :: get(SYS_FILE_PATH) . Path :: namespace_to_path(__NAMESPACE__) . '/data/' . md5(
                    serialize(
                            array($this->get_cas_parameters()->get_user_id(), 0, $action->get_id())));
        ...
        else
        {
            ...
            foreach ($action_statistics as $application_id => $application_statistics)
            {
                $path = Path :: get(SYS_FILE_PATH) . Path :: namespace_to_path(__NAMESPACE__) . '/data/' . md5(
                        serialize(
                                array($this->get_cas_parameters()->get_user_id(), $application_id, $action->get_id())));
                ...
                $sub_tabs->add_tab(
                        new DynamicContentTab(1, Translation :: get('Chart'), 
                                Theme :: get_image_path(__NAMESPACE__) . 'sub_tabs/1.png', $graph->chart()));
                $sub_tabs->add_tab(
                        new DynamicContentTab(2, Translation :: get('Table'), 
                                Theme :: get_image_path(__NAMESPACE__) . 'sub_tabs/2.png', $graph->table()));
                
                $table = new SortableTable($data);
                $table->set_header(0, Translation :: get('Date'), false);
                $sub_tabs->add_tab(
                        new DynamicContentTab(3, Translation :: get('DatesTable'), 
                                Theme :: get_image_path(__NAMESPACE__) . 'sub_tabs/3.png', $table->toHTML()));
                
                $tabs->add_tab(
                        new DynamicContentTab($application_id, $applications[$application_id]->get_title(), 
    ...
    function render()
    {
        ...
        if (! Rights :: get_instance()->module_is_allowed(Rights :: VIEW_RIGHT, $entities, 
                $this->get_module_instance()->get_id(), $this->get_cas_parameters()))
        {
            Display :: not_allowed();
        }
        ...
        if (count($this->get_cas_statistics()) > 0)
        {
            ...
            {
                $tabs->add_tab(
                        new DynamicContentTab($action->get_id(), Translation :: get($action->get_name()), 
