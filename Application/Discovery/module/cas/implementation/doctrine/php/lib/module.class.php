    ...
    function get_statistics_table($action)
    {
        ...
        else
        {
            ...
            foreach ($action_statistics as $application_id => $application_statistics)
            {
                $path = Path :: get(SYS_FILE_PATH) . Path :: namespace_to_path(__NAMESPACE__) . '/data/' . md5(
                        serialize(
                                array($this->get_cas_parameters(), $application_id, $action->get_id())));
                ...
                // $sub_tabs->add_tab(new DynamicContentTab(2, Translation :: get('Table'), Theme ::
                // get_image_path(__NAMESPACE__) . 'sub_tabs/2.png', $graph->table()));
