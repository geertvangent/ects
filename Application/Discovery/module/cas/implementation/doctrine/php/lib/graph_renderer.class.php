...
class GraphRenderer
{
    ...
    function __construct($module, $user_id, $application, $action)
    {
    ...
    function chart()
    {
        ...
        if (! file_exists($image_path . $image_file))
        {
            ...
            $graph->drawScale($this->graph_data[0], $this->graph_data[1], SCALE_START0, 150, 150, 150, TRUE, 60, 0, TRUE);
    ...
    function get_config()
    {
        ...
        $config['Description'][$this->application->get_id()] = Translation :: get($this->action->get_name()) . ' ' . $this->application->get_title();
    ...
    function get_data()
    {
        ...
        foreach ($this->months as $key => $month)
        {
            ...
            $conditions[] = new InequalityCondition(Cas :: PROPERTY_DATE, InEqualityCondition :: GREATER_THAN_OR_EQUAL, date('Y-m-d G:i:s', $first_day_of_month));
            $conditions[] = new InequalityCondition(Cas :: PROPERTY_DATE, InEqualityCondition :: LESS_THAN, date('Y-m-d G:i:s', $first_day_of_next_month));
            
            $condition = new AndCondition($conditions);
            
            $count = DataManager :: get_instance($this->module->get_module_instance())->count_cas_graph_statistics($condition);
