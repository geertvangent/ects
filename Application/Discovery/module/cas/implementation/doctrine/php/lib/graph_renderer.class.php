...
class GraphRenderer
{
    ...
    function __construct($module, $user_id, $application, $action, $statistics)
    {
        ...
        $path = Path :: get(SYS_FILE_PATH) . Path :: namespace_to_path(__NAMESPACE__) . '/graph_data/' . md5(
                serialize(array($user_id, $application, $action)));
    ...
    function chart()
    {
        ...
        if (! file_exists($image_path . $image_file))
        {
            ...
            $graph->drawScale($this->graph_data[0], $this->graph_data[1], SCALE_START0, 150, 150, 150, TRUE, 60, 0,
                    TRUE);
    ...
    function get_months()
    {
        ...
        {
            $path = Path :: get(SYS_FILE_PATH) . Path :: namespace_to_path(__NAMESPACE__) . '/months/' . md5(
                    serialize(array($this->user_id, $this->action, $this->application)));

            if (! file_exists($path))
            {
                $first_date = DataManager :: get_instance($this->module->get_module_instance())->retrieve_first_date(
                        $this->user_id, $this->action, $this->application);
