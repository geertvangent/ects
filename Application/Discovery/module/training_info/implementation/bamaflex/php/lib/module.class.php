...
class Module extends \application\discovery\module\training_info\Module
{
    ...
    function render()
    {
        ...
        $tabs->add_tab(new DynamicVisualTab(self :: TAB_GOALS, Translation :: get('Goals'), Theme :: get_image_path() . 'tabs/' . self :: TAB_GOALS . '.png', $this->get_instance_url($this->get_module_instance()->get_id(), $parameters), $current_tab == self :: TAB_GOALS));
        $parameters->set_tab(self :: TAB_OPTIONS);
        $tabs->add_tab(new DynamicVisualTab(self :: TAB_OPTIONS, Translation :: get('Options'), Theme :: get_image_path() . 'tabs/' . self :: TAB_OPTIONS . '.png', $this->get_instance_url($this->get_module_instance()->get_id(), $parameters), $current_tab == self :: TAB_OPTIONS));
        $parameters->set_tab(self :: TAB_TRAJECTORIES);
        $tabs->add_tab(new DynamicVisualTab(self :: TAB_TRAJECTORIES, Translation :: get('Trajectories'), Theme :: get_image_path() . 'tabs/' . self :: TAB_TRAJECTORIES . '.png', $this->get_instance_url($this->get_module_instance()->get_id(), $parameters), $current_tab == self :: TAB_TRAJECTORIES));
        $parameters->set_tab(self :: TAB_COURSES);
        $tabs->add_tab(new DynamicVisualTab(self :: TAB_COURSES, Translation :: get('Courses'), Theme :: get_image_path() . 'tabs/' . self :: TAB_COURSES . '.png', $this->get_instance_url($this->get_module_instance()->get_id(), $parameters), $current_tab == self :: TAB_COURSES));
