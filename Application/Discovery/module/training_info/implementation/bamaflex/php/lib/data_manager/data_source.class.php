...
class DataSource extends \application\discovery\connection\bamaflex\DataSource implements DataManagerInterface
{
    ...
    function retrieve_courses($parameters)
    {
        ...
        if (! isset($this->courses[$id][$source]))
        {
            ...
            $query .= 'WHERE parent_id IS NULL AND training_id = "' . $id . '" AND source = "' . $source . '" ORDER BY name';
            ...
            if (! $results instanceof MDB2_Error)
            {
                while ($result = $results->fetchRow(MDB2_FETCHMODE_OBJECT))
                {
                    ...
                    $course->set_children($this->retrieve_courses_children($id, $source, $course->get_id()));
                    
                    $this->courses[$id][$source][] = $course;
                }
            }
        }
    ...
    function retrieve_groups($training_parameters)
    {
        ...
        {
            $query = 'SELECT * FROM [dbo].[v_discovery_training_group_advanced] ';
            $query .= 'WHERE training_id = "' . $training_id . '" AND source = "' . $source . '"';
    ...
    function retrieve_sub_trajectory_courses_children($id, $source, $parent_programme_id)
    {
        if (! isset($this->sub_trajectory_courses_children[$id][$source][$parent_programme_id]))
        {
            $query = 'SELECT * FROM [dbo].[v_discovery_training_info_sub_trajectory_course_advanced] ';
    ...
    function retrieve_courses_children($id, $source, $parent_programme_id)
    {
        if (! isset($this->courses_children[$id][$source][$parent_programme_id]))
        {
            $query = 'SELECT * FROM [dbo].[v_discovery_course_advanced] ';
            $query .= 'WHERE parent_id = "' . $parent_programme_id . '" AND training_id = "' . $id . '" AND source = "' . $source . '"';
            ...
            if (! $results instanceof MDB2_Error)
            {
                while ($result = $results->fetchRow(MDB2_FETCHMODE_OBJECT))
                {
                    $package_course = new Course();
                    $package_course->set_id($result->id);
                    $package_course->set_training_id($result->training_id);
                    $package_course->set_name($this->convert_to_utf8($result->name));
                    $package_course->set_source($result->source);
                    $package_course->set_credits($result->credits);
                    
                    $this->courses_children[$id][$source][$parent_programme_id][] = $package_course;
                }
            }
        }
        
        return $this->courses_children[$id][$source][$parent_programme_id];
    }
