...
class Module extends \application\discovery\module\course\Module
{
    ...
    function render()
    {
        ...
        $tabs->add_tab(
                new DynamicContentTab(self :: TAB_GENERAL, Translation :: get('General'), 
                        Theme :: get_image_path() . 'tabs/' . self :: TAB_GENERAL . '.png', $this->get_general()));
        if ($course->has_materials())
        {
            $tabs->add_tab(
                    new DynamicContentTab(self :: TAB_MATERIALS, Translation :: get('Materials'), 
                            Theme :: get_image_path() . 'tabs/' . self :: TAB_MATERIALS . '.png', $this->get_materials()));
        }
        if ($course->has_activities())
        {
            $tabs->add_tab(
                    new DynamicContentTab(self :: TAB_ACTIVITIES, Translation :: get('Activities'), 
                            Theme :: get_image_path() . 'tabs/' . self :: TAB_ACTIVITIES . '.png', 
                            $this->get_activities()));
        }
        if ($course->has_competences())
        {
            $tabs->add_tab(
                    new DynamicContentTab(self :: TAB_COMPETENCES, Translation :: get('Competences'), 
                            Theme :: get_image_path() . 'tabs/' . self :: TAB_COMPETENCES . '.png', 
                            $this->get_competences()));
        }
        if ($course->has_content())
        {
            $tabs->add_tab(
                    new DynamicContentTab(self :: TAB_CONTENT, Translation :: get('Content'), 
                            Theme :: get_image_path() . 'tabs/' . self :: TAB_CONTENT . '.png', $this->get_content()));
        }
        
        $tabs->add_tab(
                new DynamicContentTab(self :: TAB_EVALUATIONS, Translation :: get('Evaluations'), 
                        Theme :: get_image_path() . 'tabs/' . self :: TAB_EVALUATIONS . '.png', $this->get_evaluations()));
        ...
        if ($course->has_coordinators())
        {
            if ($teaching_assignment_module_instance)
            {
                ...
                foreach ($course->get_teachers() as $coordinator)
                {
                    ...
                    {
                        $user = UserDataManager :: get_instance()->retrieve_user_by_official_code(
