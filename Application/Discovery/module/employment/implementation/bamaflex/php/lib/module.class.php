...
//             $row[] = $employment->get_pay_scale();
            ...
            if (count($parts) > 1)
            {
                foreach ($parts as $part)
                {
                    ...
                    $row[] = '<span class="employment_part">' . $part->get_start_date() . '</span>';
                    $row[] = '<span class="employment_part">' . $part->get_end_date() . '</span>';
                    $data[] = $row;
                }
            }
...
//         $table->set_header(7, Translation :: get('State'), false);
        $table->set_header(7, Translation :: get('Fund'), false);
//         $table->set_header(8, Translation :: get('PayScale'), false);
