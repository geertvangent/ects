...
class InstanceForm extends FormValidator
{
    ...
    public function build_basic_form()
    {
        ...
        $tabs_generator->add_tab(
            new DynamicFormTab(
                'general',
                'General',
                Theme :: get_common_image_path() . 'place_tab_view.png',
                'build_general_form'));
        ...
        if (count($configuration['settings']) > 0)
        {
            $tabs_generator->add_tab(
                new DynamicFormTab(
                    'settings',
                    'Settings',
                    Theme :: get_common_image_path() . 'place_tab_settings.png',
                    'build_settings_form'));
        }
    ...
    public function build_general_form()
    {
        ...
        $this->addElement(
            ...
            Translation :: get('Title', null, $this->instance->get_type()),
            array("size" => "50"));
        $this->addRule(
            Instance :: PROPERTY_TITLE,
    ...
    public function build_settings_form()
    {
        ...
        $path = Path :: namespace_to_path($instance->get_type());
                ...
                if (! $has_settings && $categories > 1)
                {
                    $this->addElement(
                        ...
                        Translation :: get(
                            ...
                            null,
                            Manager :: get_namespace($this->instance->get_instance_type(), $this->instance->get_type())));
                    $has_settings = true;
                }
    ...
    public function update_instance()
    {
        ...
        $instance->set_title($values[Instance :: PROPERTY_TITLE]);
        $instance->set_description($values[Instance :: PROPERTY_DESCRIPTION]);

        if (isset($values[self :: PROPERTY_ENABLED]))
        {
            $instance->set_content_type($instance->get_module_type());
        }
        else
        {
            $instance->set_content_type(Instance :: TYPE_DISABLED);
        }
    ...
    public function setDefaults($defaults = array ())
    {
        ...
        if (! $instance->get_title())
        {
            $defaults[Instance :: PROPERTY_TITLE] = Translation :: get('TypeName', null, $this->instance->get_type());
        }
        else
        {
            $defaults[Instance :: PROPERTY_TITLE] = $instance->get_title();
        }
        ...
        if ($instance->get_title())
        {
            $defaults[self :: PROPERTY_ENABLED] = (int) $instance->is_enabled();
        }
        else
        {
    ...
    public function get_instance_types()
    {
        $path = Path :: get_common_extensions_path() . 'instance_manager/implementation/';
        $folders = Filesystem :: get_directory_content($path, Filesystem :: LIST_DIRECTORIES, false);

        $types = array();
        foreach ($folders as $folder)
        {
            $types[$folder] = Translation :: get('TypeName', null, Manager :: get_namespace($folder));
        }
        ksort($types);
        return $types;
    }
