...
class DiscoveryComponent extends Manager implements DelegateComponent
{
    ...
    public function run()
    {
        try
        {
            ...
            $application = \application\atlantis\application\DataManager :: retrieve(\application\atlantis\application\Application :: class_name(), $parameters);

            $condition = new EqualityCondition(\application\atlantis\application\right\Right :: PROPERTY_APPLICATION_ID, $application->get_id());
            ...
            {
                $condition = new EqualityCondition(\application\discovery\ModuleInstance :: PROPERTY_TYPE, $right->get_code());
                ...
                {
                    $condition = new EqualityCondition(\application\atlantis\role\entitlement\Entitlement :: PROPERTY_RIGHT_ID, $right->get_id());
                    $parameters = DataClassRetrievesParameters :: generate($condition);
                    $entitlements = \application\atlantis\role\entitlement\DataManager :: retrieves(\application\atlantis\role\entitlement\Entitlement :: class_name(), $parameters);

                    while ($entitlement = $entitlements->next_result())
                    {
                        $condition = new EqualityCondition(\application\atlantis\role\entity\RoleEntity :: PROPERTY_ROLE_ID, $entitlement->get_role_id());
                        $parameters = DataClassRetrievesParameters :: generate($condition);
                        $entities = \application\atlantis\role\entity\DataManager :: retrieves(\application\atlantis\role\entity\RoleEntity :: class_name(), $parameters);
