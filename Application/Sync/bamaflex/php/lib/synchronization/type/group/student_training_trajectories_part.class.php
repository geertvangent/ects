...
class StudentTrainingTrajectoriesPartGroupSynchronization extends GroupSynchronization
{
    ...
    function get_user_official_codes()
    {
        ...
        $query = 'SELECT DISTINCT person_id FROM [dbo].[v_discovery_list_user_student_basic]  WHERE type = 1 AND result != 8 AND trajectory_part = "' . $this->get_parameter(self :: RESULT_PROPERTY_TRAJECTORY_PART) . '"';
