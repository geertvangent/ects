...
abstract class CourseGroupSynchronization extends GroupSynchronization
{
    ...
    public function get_children()
    {
        ...
        if (is_null($this->get_parameter(self :: RESULT_PROPERTY_PARENT_ID)) && $this->get_parameter(self :: RESULT_PROPERTY_TYPE) == 2)
        {
            $query = 'SELECT * FROM [INFORDATSYNC].[dbo].[v_discovery_course_basic] WHERE parent_id = ' . $this->get_parameter(self :: RESULT_PROPERTY_COURSE_ID) . ' AND exchange = 0';
