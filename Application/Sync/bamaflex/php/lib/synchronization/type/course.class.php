...
class CourseSynchronization extends Synchronization
{
    ...
    function synchronize($course)
    {
        ...
        if (! $current_course instanceof Course)
        {
            ...
            {
                $this->course_categories_cache[$category_code] = WeblcmsDataManager :: get_instance()->retrieve_course_category_by_code(
                        $category_code);
            }
            ...
            if ($new_course->create())
            {
                ...
                {
                    WeblcmsDataManager :: get_instance()->subscribe_group_to_course($new_course, $group->get_id(),
        ...
        else
        {
            ...
            if ($group instanceof \group\Group)
            {
                if (! WeblcmsDataManager :: get_instance()->is_group_subscribed($current_course->get_id(),
                        $group->get_id()))
                {
                    WeblcmsDataManager :: get_instance()->subscribe_group_to_course($current_course, $group->get_id(),
                            CourseGroupRelation :: STATUS_TEACHER);
