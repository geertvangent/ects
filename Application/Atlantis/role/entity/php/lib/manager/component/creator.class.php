...
class CreatorComponent extends Manager
{
    ...
    function run()
    {
        ...
        if ($form->validate())
        {
                ...
                foreach ($entity_ids as $entity_id)
                {
                    foreach ($values['role']['role'] as $role)
                    {
                        foreach ($values['context']['context'] as $context)
                        {
                            ...
                            $entity->set_entity_type($entity_type == 'user' ? UserEntity :: ENTITY_TYPE : PlatformGroupEntity :: ENTITY_TYPE);
            ...
            $count = (count($values['entity'][UserEntity :: ENTITY_TYPE]) + count($values['entity'][PlatformGroupEntity :: ENTITY_TYPE])) * count($values['role']) * count($values['context']);
