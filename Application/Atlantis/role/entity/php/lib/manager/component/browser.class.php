...
class BrowserComponent extends Manager implements NewObjectTableSupport, DelegateComponent
{
    ...
    function add_breadcrumb()
    {
        if ($this->has_role_id())
        {
            BreadcrumbTrail :: get_instance()->add(new Breadcrumb(null, \application\atlantis\role\DataManager :: retrieve(\application\atlantis\role\Role :: class_name(), (int) $this->role_id)->get_name()));
        }
        if ($this->has_context_id())
        {
            BreadcrumbTrail :: get_instance()->add(new Breadcrumb(null, \application\atlantis\context\DataManager :: retrieve(\application\atlantis\context\Context :: class_name(), (int) $this->context_id)->get_context_name()));
        }
        if ($this->has_entity())
        {
            BreadcrumbTrail :: get_instance()->add(new Breadcrumb(null, RoleEntity :: entity_name($this->entity_type, $this->entity_id)));
        }
    }
