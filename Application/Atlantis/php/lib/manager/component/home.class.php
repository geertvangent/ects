...
class HomeComponent extends Manager implements DelegateComponent
{
    ...
    public function run()
    {
        ...
        $actions = array();
        $actions[] = new DynamicSearchAction($namespace, 
                $this->get_url(array(self :: PARAM_ACTION => self :: ACTION_ROLE)));
        $actions[] = new DynamicAction(Translation :: get('BrowseRoles', null, $namespace), 
                Translation :: get('BrowseRolesDescription', null, $namespace), 
                Theme :: get_image_path($namespace) . 'admin/browse.png', 
                $this->get_url(array(self :: PARAM_ACTION => self :: ACTION_ROLE)));
        if ($this->get_user()->is_platform_admin())
        {
            $actions[] = new DynamicAction(Translation :: get('CreateRole', null, $namespace), 
        ...
        $tabs->add_tab(
                new DynamicActionsTab('role', Translation :: get('TypeName', null, $namespace), 
                        Theme :: get_image_path($namespace) . 'logo/22.png', $actions));
        ...
        $actions[] = new DynamicAction(Translation :: get('BrowseApplications', null, $namespace), 
                Translation :: get('BrowseApplicationsDescription', null, $namespace), 
                Theme :: get_image_path($namespace) . 'admin/browse.png', 
                $this->get_url(array(self :: PARAM_ACTION => self :: ACTION_APPLICATION)));
        if ($this->get_user()->is_platform_admin())
        {
            $actions[] = new DynamicAction(Translation :: get('CreateApplication', null, $namespace), 
                    Translation :: get('CreateApplicationDescription', null, $namespace), 
                    Theme :: get_image_path($namespace) . 'admin/create.png', 
                    $this->get_url(
                            array(self :: PARAM_ACTION => self :: ACTION_APPLICATION, 
                                    \application\atlantis\application\Manager :: PARAM_ACTION => \application\atlantis\application\Manager :: ACTION_CREATE)));
        }
        $tabs->add_tab(
                new DynamicActionsTab('application', Translation :: get('TypeName', null, $namespace), 
                        Theme :: get_image_path($namespace) . 'logo/22.png', $actions));
        
        // RoleEntity tab
        $namespace = \application\atlantis\role\entity\Manager :: context();
        $actions = array();
        $actions[] = new DynamicAction(Translation :: get('BrowseRoleEntities', null, $namespace), 
        ...
        if ($this->get_user()->is_platform_admin())
        {
            $actions[] = new DynamicAction(Translation :: get('CreateRoleEntity', null, $namespace), 
        ...
        $tabs->add_tab(
                new DynamicActionsTab('role_entity', Translation :: get('TypeName', null, $namespace), 
                        Theme :: get_image_path($namespace) . 'logo/22.png', $actions));
        
        // // Entity type
        // $namespace = \application\atlantis\role\entity\Manager :: context();
        // $actions = array();
        // $actions[] = new DynamicAction(Translation :: get('BrowseUsersGroups', null, $namespace), Translation ::
        // get('BrowseUsersGroupsDescription', null, $namespace), Theme :: get_image_path($namespace) .
        // 'admin/browse.png', $this->get_url(array(
        ...
        $actions[] = new DynamicAction(Translation :: get('BrowseContexts', null, $namespace),
                Translation :: get('BrowseContextsDescription', null, $namespace),
                Theme :: get_image_path($namespace) . 'admin/browse.png',
                $this->get_url(array(self :: PARAM_ACTION => self :: ACTION_CONTEXT)));
        
        $tabs->add_tab(
                new DynamicActionsTab('context', Translation :: get('TypeName', null, $namespace),
                        Theme :: get_image_path($namespace) . 'logo/22.png', $actions));
